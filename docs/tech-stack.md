...
这套栈不是为了“显得完整”而堆技术。v1 核心仍然是：Workers + D1 + Drizzle + Hono + React。R2、Vectorize、Workers AI、Queues/Cron 只在对应功能出现时启用。
...
- KV：只有出现明确缓存或配置场景时再加。
- Durable Objects：只有实时同步、协作、WebSocket、用户级协调或强一致限流真的需要时再加。
- Queues / Cron：只有链接预览刷新、导出生成、embedding、清理、定期回顾等异步任务出现时再加。
- Vectorize：只有实现语义搜索时再加。
- Workers AI：只有实现 AI 功能时再加。
...
- 第一版产品保持克制：快速收集、时间线、搜索、标签、导入导出和 Memos 兼容核心端点。
